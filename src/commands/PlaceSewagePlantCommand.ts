...
import { BUILDING_SEWAGE_PLANT } from '../sim/constants';
import { BALANCE } from '../data/balance';

export class PlaceSewagePlantCommand extends Command {
  ...
  execute(world: World): boolean {
    ...
    if (world.budget.money < BALANCE.costs.sewagePlant) return false;
    this.snap = snapshotTile(world, this.tx, this.ty);
    this.cost = BALANCE.costs.sewagePlant;
    world.budget.money -= this.cost;
    const ok = world.placeBuilding(this.tx, this.ty, BUILDING_SEWAGE_PLANT);
    if (ok) world.events.emit('sewagePlantBuilt', { tx: this.tx, ty: this.ty, cost: this.cost });
    return ok;
  }
