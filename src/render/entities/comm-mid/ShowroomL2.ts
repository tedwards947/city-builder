 ...
 * A sleek, modern 1-story showroom with a massive glass facade
 * and a minimalist architectural style.
 */
export const ShowroomL2: VectorEntity = {
  ...
  draw: (ctx, ts, t, p, vibe) => {
    ...
    for (let x = bx + 4 + Math.floor(fw / 3); x < bx + bw - 4; x += Math.floor(fw / 3)) {
        ctx.fillRect(x, by + 4, 1, fh);
    }

    // 4. Interior Display (Simplified "Cars" or goods)
    const displayColors = ['#e53e3e', '#3182ce', '#38a169'];
    for (let i = 0; i < 3; i++) {
        const dx = bx + 10 + i * Math.floor(fw / 3.5);
        const dy = by + bh - 12;
    ...
    // Recessed Lighting (Small yellow dots under roof)
