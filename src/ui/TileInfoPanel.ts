...
function analyzeZone(world: World, tx: number, ty: number): { level: number; maxLevel: number; reqs: Req[] } {
  ...
  if (dev >= 2) {
    ...
    if (zone === ZONE_R) {
      reqs.push({
        label: t('ui.tileInfo.eduLevel', { value: education, threshold: BALANCE.education.growthThreshold }),
  ...
  if (zone === ZONE_R) {
    ...
    reqs.push({
      label: t('ui.tileInfo.sickBelowThreshold', { value: sickness, threshold: BALANCE.healthcare.growthThreshold }),
      met: sickness <= BALANCE.healthcare.growthThreshold,
    });
  }
  if (zone !== ZONE_I) {
    reqs.push({
      label: t('ui.tileInfo.pollBelowThreshold', { value: pollution, threshold: BALANCE.pollution.growthThreshold }),
      met: pollution <= BALANCE.pollution.growthThreshold,
    });
    reqs.push({
      label: t('ui.tileInfo.crimeBelowThreshold', { value: crime, threshold: BALANCE.crime.growthThreshold }),
...
export class TileInfoPanel {
  ...
  private _render(world: World, tx: number, ty: number): void {
    ...
    // ── Building tile ──────────────────────────────────────────────────────────
    if (building !== 0) {
      const name = BUILDING_NAME[building] ? t(BUILDING_NAME[building]) : `${t('buildings.building')} #${building}`;
    ...
    // ── Road tile ──────────────────────────────────────────────────────────────
      ...
      const congKey = cong < 30  ? 'congestion.free'
                      : cong < 85  ? 'congestion.light'
                      : cong < 140 ? 'congestion.moderate'
                      : cong < 200 ? 'congestion.heavy'
                      :              'congestion.gridlock';
      ...
      const congColor = cong < 30  ? '#5c5'
                      ...
                      : cong < 200 ? '#e95'
                      :              '#e55';
      html += `<div class="ti-type">${t('zones.road')}</div>`;
    ...
    // ── Zone tile ──────────────────────────────────────────────────────────────
      ...
      const lvKey = landValue < 60 ? 'landValue.low' : landValue < 110 ? 'landValue.medium' : landValue < 160 ? 'landValue.good' : 'landValue.prime';
      const lvLabel = t(lvKey);
      html += `<div class="ti-detail">${t('ui.tileInfo.landValue', { value: landValue, label: lvLabel })}</div>`;

      if (pollution > 0) {
        const pKey = pollution < 40 ? 'common.low' : pollution < 100 ? 'common.medium' : 'common.high';
        const pLabel = t(pKey);
        html += `<div class="ti-detail ti-poll">${t('ui.tileInfo.pollution', { value: pollution, label: pLabel })}</div>`;
      }

      if (crime > 0) {
        const cKey = crime < 40 ? 'common.low' : crime < 100 ? 'common.medium' : 'common.high';
        const cLabel = t(cKey);
      ...
      if (zone === ZONE_R) {
        ...
        const eduKey = edu < 50 ? 'education.uneducated' : edu < 120 ? 'education.basic' : edu < 200 ? 'education.educated' : 'education.highlyEducated';
        ...
        if (sick > 0) {
          const sickKey = sick < 80 ? 'sickness.mild' : sick < 160 ? 'sickness.moderate' : 'sickness.severe';
      ...
      if (fireRisk > 0) {
        const fKey = fireRisk < 40 ? 'common.low' : fireRisk < 100 ? 'common.medium' : 'common.high';
        const fLabel = t(fKey);
      ...
      if (isAbandoned) {
        ...
        for (const req of reqs) {
          const icon = req.met ? `<span class="ti-yes">${t('common.yes')}</span>` : `<span class="ti-no">${t('common.no')}</span>`;
    ...
    // ── Empty tile ─────────────────────────────────────────────────────────────
      ...
      const terrainName = TERRAIN_NAME[terrain] ? t(TERRAIN_NAME[terrain]) : t('terrain.unknown');
      if (vegetation !== VEG_NONE) {
        const treeName = TREE_SPECIES[vegetation] ? t(TREE_SPECIES[vegetation]) : t('trees.trees');
